...
HOST = '127.0.0.1' # Set host as localhost by default - shouldn't need to change this for the assignment, but might be funny to play with later.
# Send/Recv port.
PORT = 55055
# Name (pseud) of the system.
NAME = "Server: "
...
def msg(args=[]):
    ...
    if CONN is not None:
        ...
        while mremainder != 0:
            ...
            CONN.sendall(bytes(mpack, 'ascii'))
    print(NAME + ">> ", end="")
...
def system_connect(args=[]):
    global CONN, ADDR, SOCKET_HANDLE, NAME
    ...
    print(NAME + " Connection established to '" + str(ADDR) + "'.")
...
# Bind the hotkey 'ctrl plus shift' to the command interface trigger for the server CI. This is how we will let users type things into the console.
keyboard.add_hotkey('ctrl+shift', command_interface)
...
print(NAME + "Server is configured to spin up using SEND/RECV address of " + HOST + ":" + str(PORT))
print(NAME + "You may override this using /i <address> and /p <portnumber>.")
print(NAME + "For command list, enter /h. To enable command capture at any time press 'lctrl+lshift'. Exit with /x.")
